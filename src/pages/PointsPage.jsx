  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-4 pt-24 pb-24 flex flex-col gap-6">
      <div className="flex items-center justify-between sticky top-20 z-40 bg-light-white/95 backdrop-blur-sm py-4 -mx-4 px-4 rounded-lg">
        <h1 className="font-aleo text-2xl font-bold text-gray-900">Points & Rewards</h1>
      ...
      {!account && (
        <Card className="rounded-3xl border-2 border-dashed border-primary/40 bg-primary-50/50 shadow-lg">
          <CardBody className="p-8 text-center">
            <p className="text-gray-700 font-medium mb-2">Connect your Leo Wallet to view your points</p>
            <p className="text-sm text-gray-500 mb-4">Points are tied to your Aleo (Leo) wallet. Connect the same wallet you use for payments to see your balance and history.</p>
            <div className="flex justify-center">
              <WalletMultiButton className="!bg-primary hover:!bg-primary-800 !text-white !font-bold !h-12 !rounded-2xl !px-6" />
            </div>
          </CardBody>
        </Card>
      )}
      ...
      <Card className="rounded-3xl border border-gray-200 shadow-lg">
        <CardBody className="p-6">
          <h2 className="font-aleo text-xl font-semibold mb-4">Points History</h2>
          {isLoading ? (
            <div className="flex justify-center py-8">
              <Spinner color="primary" />
            </div>
          ) : pointsHistory.length === 0 ? (
            <p className="text-gray-500 text-center py-8">No points history yet. Send or receive ALEO and create payment links to earn points.</p>
          ) : (
              ...
              <TableBody>
                {pointsHistory.map((tx) => (
                  <TableRow key={tx.id}>
                    ...
                    <TableCell>
                      <span className="font-semibold text-green-600">+{tx.points}</span>
                    </TableCell>
      ...
      <Card className="rounded-3xl border border-gray-200 shadow-lg">
        <CardBody className="p-6">
          <h2 className="font-aleo text-xl font-semibold mb-4">How to Earn Points</h2>
              ...
              {pointsConfig.map((config) => (
                ...
                >
                  <div className="flex-1">
                    <p className="font-medium text-gray-900">{config.description || config.action_type?.replace(/_/g, " ")}</p>
      ...
      <Card className="rounded-3xl border border-gray-200 shadow-lg">
        <CardBody className="p-6">
          <h2 className="font-aleo text-xl font-semibold mb-4">Top Users</h2>
          <p className="text-sm text-gray-600 mb-4">Ranked by lifetime points (Aleo)</p>

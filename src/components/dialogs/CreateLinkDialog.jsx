...
import { ALEO_LOGO_PRIMARY } from "../../config.js";
...
function StepOne({
  ...
  async function handleUpdate() {
    ...
    try {
      ...
      const currentUsername = localStorage.getItem(`aleo_username_${account}`) || account?.slice(-8) || "user";
    ...
    try {
      if (navigator.share) {
        await navigator.share({
          title: "Payment Link",
          text: `Pay me at ${alias}.privatepay.me`,
          url: paymentPageUrl,
        });
      ...
      <div className="relative w-full h-full mt-5">
        ...
        <div className="relative w-full h-52 md:h-60 flex flex-col items-center justify-start py-7 px-6">
          ...
          <div className="absolute right-5 bottom-6 flex items-center justify-between">
            <img src={ALEO_LOGO_PRIMARY} alt="Aleo" className="w-12 h-auto object-contain" />
          </div>
        </div>
      </div>

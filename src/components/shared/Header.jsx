    ...
    <nav className="fixed top-0 z-50 flex items-center px-5 md:px-12 h-20 justify-between bg-white md:bg-transparent w-full">
      ...
      <div className="flex gap-4 items-center justify-center">
        ...
        >
          ...
          <h1 className="font-aleo text-sm font-medium text-white">Create Link</h1>
        </Button>
...
const UserProfileButton = () => {
  const { account, isConnected, connect, disconnect, wallet } = useAppWallet();
  const { setVisible } = useWalletModal();
  const [showMenu, setShowMenu] = useState(false);

  const handleConnect = async () => {
    // If no wallet is selected yet, open the wallet modal so user can pick Leo Wallet first.
    // Calling connect() without a selected wallet throws WalletNotSelectedError.
    if (!wallet) {
      setVisible(true);
      return;
    }
            ...
            <div className="flex flex-col gap-3">
              <div className="flex items-center gap-3 pb-3 border-b border-neutral-200">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500">Aleo Wallet</p>
                </div>
              </div>
              ...
              <button
                onClick={() => {
                  window.open(
                    `https://testnet.explorer.provable.com/address/${account}`,
                    "_blank"
                  );
                  setShowMenu(false);
                }}

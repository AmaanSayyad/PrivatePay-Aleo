...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function loadBalance() {
      if (account) {
        const username = localStorage.getItem(`aleo_username_${account}`) || account?.slice(-8) || "user";
        ...
        if (previousBalanceRef.current > 0 && newBalance > previousBalanceRef.current) {
          ...
          if (hasPermission) notifyPaymentReceived(`${received} ALEO`, null);
        }
...
function ReceiveCard({ setOpenQr, user, isLoading }) {
  ...
  useEffect(() => {
    if (account) {
      const savedUsername = localStorage.getItem(`aleo_username_${account}`);
      if (savedUsername) {
...
function MergedBalanceCard({ balance, isLoading }) {
  ...
  return (
    <div className="w-full rounded-3xl bg-gradient-to-br from-white to-primary-50/30 border border-neutral-200 shadow-sm overflow-hidden relative">
      ...
      <div className="w-full px-6 pt-6 pb-4">
        ...
        {isLoading ? (
          ...
          <div className="flex items-baseline gap-2 mb-2">
            ...
            <p className="text-lg font-semibold text-gray-600">ALEO</p>
          </div>
        )}
      </div>
      ...
      {/* Action Buttons — Send, Withdraw (from treasury), History */}

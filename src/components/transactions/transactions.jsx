...
export default function Transactions() {
  ...
  const loadTransactions = async () => {
    ...
    try {
      const savedUsername = localStorage.getItem(`aleo_username_${account}`) || account?.slice(-8) || "";
      ...
      {isLoading ? (
            ...
            <div key={date} className="mb-4">
              ...
              {groupedTransactions[date].map((tx, idx) => {
                ...
                if (isWithdrawal) {
                  ...
                  value = `-${Math.abs(tx.amount).toFixed(4)} ALEO`;
                } else if (isSent) {
                  title = "Sent Payment";
        ...
        <div className="w-full flex flex-col items-center justify-center min-h-64 gap-2">
          ...
          <p className="text-sm text-gray-400">
            Transactions will appear here when you receive or send ALEO
          </p>
        </div>
      )}
    </div>

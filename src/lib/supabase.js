...
export async function recordPayment(senderAddress, recipientUsername, amount, txHash) {
  ...
  try {
    ...
    // Points: award to sender and recipient (Aleo) — use trimmed wallet for consistency
    try {
      const senderWallet = (senderAddress || '').trim();
      if (senderWallet) {
        await awardPoints(senderWallet, 'payment_sent', { description: `Payment sent: ${amount} ALEO`, relatedPaymentId: payment?.id });
      ...
      if (recipientWallet) {
        await awardPoints(recipientWallet, 'payment_received', { description: `Payment received: ${amount} ALEO`, relatedPaymentId: payment?.id });
...
export async function updateUsername(walletAddress, newUsername) {
  ...
  if (typeof window !== 'undefined') {
    localStorage.setItem(`aleo_username_${walletAddress}`, normalized);
  }
  return user;
}

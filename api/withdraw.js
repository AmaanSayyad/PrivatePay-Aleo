 ...
 * Withdraw from treasury to user's wallet. Relayer logic (treasury key, Aleo transfer)
...
export default async function handler(req, res) {
  ...
  if (!treasuryKey) {
    return res.status(501).json({
      ...
      message:
        'Withdrawal relayer is not configured. Set TREASURY_PRIVATE_KEY in Vercel env and implement the Aleo transfer in api/withdraw.js (see docs/TREASURY_RELAYER.md).',
    });
  }
